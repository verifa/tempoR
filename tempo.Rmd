...
user.summary <- tempo.data %>%
  ...
  mutate(
    ...
    roll.7.hours = slide_index_dbl(hours, date, sum, .before = days(6), .complete = FALSE),
    roll.7.billable = slide_index_dbl(billable, date, sum, .before = days(6), .complete = FALSE)
  )

user.delta <- tempo.data %>%
  group_by(user) %>%
  summarise(
    start = min(startDate),
    end = max(startDate),
...
rolling <- ggplot(data = user.data) +
  geom_point(data = user.summary, aes(x = date, y = roll.7.hours), color = "Gray25") + 
  geom_smooth(data = user.summary, aes(x = date, y = roll.7.hours), color = "Gray25") + 
  geom_point(data = user.summary, aes(x = date, y = roll.7.billable), color = "Gray50") + 
  geom_smooth(data = user.summary, aes(x = date, y = roll.7.billable), color = "Gray50") + 
  facet_wrap(~user) + scale_fill_hue(l = 45) + scale_color_hue(l = 45) +
  scale_y_continuous(breaks = c(0,10,20,30,40,50), name = "Rolling Weekly [h]", 
                     sec.axis = sec_axis(~ . * 2.5, breaks = c(0, 25, 50, 75), name = "Rolling Fraction [%] ")) +
  theme(legend.position = "top", legend.title = element_blank())
```

```{r accumulated, echo = FALSE, warning = FALSE, message = FALSE}
accumulated <- ggplot(data = user.delta) +
  geom_col(aes(x = user, y = delta, fill = user)) +
