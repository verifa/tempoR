...
for (team.id in teams.with.data) {
  ...
  user.data <- subset(aggregated.data, team == team.id) %>%
  ...
  user.detail <- subset(aggregated.data, team == team.id) %>%
  ...
  user.summary <- subset(aggregated.data, team == team.id) %>%
    group_by(date, user) %>%
    summarise(
      hours = sum(hours),
      billable = sum(billable)
  ...
  user.delta <- subset(aggregated.data, team == team.id) %>%
    group_by(user) %>%
    summarise(
      start = min(startDate),
      end = max(startDate),
