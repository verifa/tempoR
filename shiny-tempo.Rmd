...
if (TEMPO_DETAILS != '') {
  renderPlot({
    if ( input$team.id != "" ) {
      user.detail <- gen.user.detail(aggregated.data, input$team.id)
      tempo.billable <- tempo.billable.plot(user.detail)
      plot(tempo.billable)
    }
  })
}
...
if (TEMPO_DETAILS != '') {
  renderPlot({
    if ( input$team.id != "" ) {
      user.detail <- gen.user.detail(aggregated.data, input$team.id)
      tempo.unbillable <- tempo.unbillable.plot(user.detail)
